...
export async function createEslintConfigIfNeeded(): Promise<ConfigCreateResponse> {
  ...
  const { files, directory } = recursivelyFindFilesUp(
    filesToSearch,
    process.cwd()
  )
  if (files.length === 0)
    throw new Error(
      `no package.json or .eslintrc file found, checked up to: ${process.cwd()}`
    )
  ...
  if (hasPackageJson) {
    const writeResult = await writeFile(
      directory,
      '.eslintrc',
      JSON.stringify({ extends: ['@sunbeams'] }, null, 2)
    )
    if (writeResult === true)
      return { configPath: path.join(directory, '.eslintrc'), directory }
    else
      throw new Error(`unable to create .eslintrc file: ${writeResult.message}`)
  }
